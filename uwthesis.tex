...
% fox@washington.edu
%
%    Revised for version 2012/02/03 of uwthesis.cls
%
...
\documentclass [11pt, twoside] {uwthesis}[2012/02/03]
...
\hypersetup{%
	pdfauthor = {Jim Fox},
	pdftitle = {The Suitability of the \LaTeX\ Text Formatter for Thesis Preparation by Technical and Non-technical Degree Candidates},
	pdfsubject = {},
	pdfkeywords = {},
	pdfcreator = {University of Washington, UW Information Technology},
	pdfproducer = {},
	bookmarksopen = {true},
	pdfpagelayout = {TwoColumnRight}, % For the two-sided version.
}
...
\Author{Jim Fox (tweaks by Daniel Halperin)}
\Year{2012}
...
\begin{verbatim}
     ...
     \end{figure}
\end{verbatim}
\caption{This text would create a
...
\caption[Generating a facing caption page]{This text would create a
  facing caption page with the accompanying figure in the one-side style.}
\end{figure}
...
\end{demo}
The \textit{text} field is the text of the caption. The optional argument
\textit{toc} controls the caption as it will appear in the List of Figures,
Tables, etc.

To include multiple figures that are part of the same figure, use the
\texttt{subfigure} package. This produces figures numbered such as 
3.3(a), 3.3(b), etc.
...
\chapter{Running \LaTeX\texorpdfstring{\\}{}
...
\$\ pdflatex uwthesis
\end{demo}
% leaving a blank line here makes the TeX think the next
% sentence is a new paragraph. This comment stops that.
and it generally works. However, \TeX\ updates its metadata for
the Table of Contents, references to figures, etc., in stages.
To ensure that all these fields are up to date, including the
bibliography, the following sequence of commands usually works.
...
which is all the Graduate School now wants.  Even if you intend to
...
He welcomes your comments to {\tt fox@uw.edu}. But please don't bug him about the tweaks; send those to {\tt dhalperi@cs.washington.edu}.
}
